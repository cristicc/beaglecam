...
while the related kernel module is `component/rpmsgcam/rpmsg_cam.c`. The corresponding
...
It is also possible to further test updated PRU firmware without rebuilding
the rootfs and rebooting the device:

[source,sh,subs="attributes+"]
----
# Copy the firmware files to device (note this is not persisted to disk!)
$ scp {DEV_OUTPUT_DIR}/build/rootfs/target/lib/firmware/* root@10.0.1.100:/lib/firmware/

# Restart PRU cores to use the updated firmware files
